...
function genNeighbor (index) {
    const [x, y, z] = index.split(",").map(s => parseInt(s, 10));
    const ns = [];
    for (const dx of [-1, 0, 1]) {
        for (const dy of [-1, 0, 1]) {
            for (const dz of [-1, 0, 1]) {
                if ([dx, dy, dz].some(d => d != 0)) {
                    ns.push([x + dx, y + dy, z + dz].join(","));
                }
            }
        }
    }
...
//console.log(genNeighbor(`0,0,0`).length);
...
let points = parseInit(realInput);
for (let i = 0; i < 6; ++i) {
    points = cycle(points);
}

'use strict';

const rl = require('readline').createInterface({
  input: process.stdin,
  terminal: false
});

const template = /^(\w+) can fly (\d+) km\/s for (\d+) seconds, but then must rest for (\d+) seconds\.\n?/;

const race_time = 2503;

let max = 0;

rl.on('close', () => console.log(max))
  .on('line', line => {
    const result = template.exec(line);
    const name = result[1];
    const speed = Number(result[2]);
    const burst_time = Number(result[3]);
    const rest_time = Number(result[4]);
    const total_period = burst_time + rest_time;

    const score = (Math.floor(race_time / total_period) * speed * burst_time
      + Math.min(burst_time, race_time % total_period) * speed);
    console.log(name, score);
    if (score > max) {
      max = score;
    }
  });


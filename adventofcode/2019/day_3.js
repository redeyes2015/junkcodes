...
const wire1 = rawToSegments(wire1Raw);
const wire2 = rawToSegments(wire2Raw);

const isHorizontal = (seg) => seg[0].y == seg[1].y;
const isVertical = (seg) => seg[0].x == seg[1].x;

const findNearPointInOverlap = () => null;

const findCross = (seg1, seg2) => {
    let vert = seg1;
    let hori = seg2;
    if (isHorizontal(seg1)) {
        if (isHorizontal(seg2)) {
            //console.log(`${seg1} vs ${seg2} => find overlap`);
            return findNearPointInOverlap(seg1, seg2);
        }
        hori = seg1;
    }
    if (isVertical(seg2)) {
    ...
    //console.log(`${seg1} vs ${seg2} => find cross`);

    const x = vert[0].x;
    const y = hori[0].y;

    if ((x - hori[0].x) * (x - hori[1].x) < 0 &&
        (y - vert[0].y) * (y - vert[1].y) < 0) {
...
for (const seg1 of wire1) {
    for (const seg2 of wire2) {
        const cross = findCross(seg1, seg2);
        if (cross == null) {
            continue;
        }
        minDist = Math.min(minDist, cross.getManhattanDistance());
    }
}
console.log(minDist);

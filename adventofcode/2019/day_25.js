...
const fs = require('fs');
const input = fs.readFileSync('./day_25.input', 'utf-8');
...
pushLine('west');
pushLine('north');
pushLine('take festive hat');
pushLine('east');
pushLine('take prime number');
pushLine('west');
pushLine('west');
pushLine('take sand');
pushLine('east');
...
c.run();
console.log(c.outputQueue.filter(c => c < 127).map(c => String.fromCharCode(c)).join(''));

...
const findNearPointInOverlap = () => Infinity;

const findCrossSteps = (seg1, seg2) => {
    let vert = seg1;
    let hori = seg2;
    ...
    if ((x - hori[0].x) * (x - hori[1].x) < 0 &&
        ...
        return hori[2] + vert[2] + Math.abs(x - hori[0].x) + Math.abs(y - vert[0].y);
    }
    return Infinity;
};
...
for (const seg1 of wire1) {
    for (const seg2 of wire2) {
        minDist = Math.min(minDist, findCrossSteps(seg1, seg2));
    }
}
console.log(minDist);
